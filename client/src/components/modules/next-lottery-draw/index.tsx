...
const NextLotteryDraw = () => {
  ...
  const { participate, isParticipating, participateData, participateError } = useDlottery();
  const { withdrawPrize, isWithdrawingPrize, withdrawPrizeData, withdrawPrizeError } = useDlottery();
  ...
  const canClaimPrize = !isWithdrawingPrize &&
    currentDrawInfo?.completed &&
    currentDrawInfo?.winner === address;

  // Format the draw date
  const formattedNextDrawDate = currentDrawInfo?.drawTime?.toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
  ...
  if (!currentDrawInfo) {
    return (
      <div className="bg-[#1E293B] rounded-xl p-6 shadow-lg text-center">
        <div className="font-bold text-lg text-white mb-4">No lottery draws available</div>
    ...
    return (
      <div className="bg-[#1E293B] rounded-xl p-6 shadow-lg">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <div>
            <h3 className="font-bold text-xl text-white mb-1">Draw Completed</h3>
  ...
  return (
    <div className="bg-[#1E293B] rounded-xl p-6 shadow-lg">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h3 className="font-bold text-xl text-white mb-1">Next Lottery Draw</h3>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="mt-4">
          <h4 className="text-lg font-medium text-white mb-3">Current Tickets</h4>
          <TicketList />
        </div>
      </div>
    </div>
